...
{% api_property eventLog 'array' %}

```javascript
// track events performed by a particular creep
_.filter(creep.room.eventLog, {objectId: creep.id});
```

```javascript
// Find all hostile actions against your creeps and structures
_.forEach(Game.rooms, room => {
    let attackEvents = _.filter(room.eventLog, {event: EVENT_ATTACK});
...
An array of events happened on the previous tick in this room. Each event represents some game action in the following format:
...
There is some CPU cost on parsing this property when you access it for the first time, for each room individually. 
